...
class CuentaUsuarioHandler
{
    ...
    public function verificarInformacion($NIF_hash, $nombre, $apellidos, $provincia, $fecha_nacimiento, $domicilio){

        $params = array($NIF_hash, $nombre, $apellidos, $provincia, $fecha_nacimiento, $domicilio);

        $query = "EXECUTE dbo.VerificarPersona ? ? ? ? ? ?";
        $stmt = sqlsrv_query($this->connection, $query, $params);
